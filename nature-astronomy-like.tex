...
\begin{widetext}
  \begin{equation}
    ...
    \begin{aligned}
      \vect{x}(t)
      &= \exp\!\left[\mat{A}(t-t_0)\right]\vect{x}(t_0)
      + \int_{t_0}^{t}\exp\!\left[\mat{A}(t-s)\right]\vect{\eta}(s)\,\mathrm{d}s,\\
      \mat{C}(t,t')
      &= \exp\!\left[\mat{A}(t-t_0)\right]\mat{C}_0
      \exp\!\left[\mat{A}^{\mathsf{T}}(t'-t_0)\right]
      + \int_{t_0}^{\min(t,t')}\exp\!\left[\mat{A}(t-s)\right]
      \mat{Q}\exp\!\left[\mat{A}^{\mathsf{T}}(t'-s)\right]\,\mathrm{d}s.
    \end{aligned}
  \end{equation}
\end{widetext}
Here $\mat{A}$ is a constant linear evolution matrix, $\mat{C}_0$ is the
initial covariance, and $\mat{Q}$ is the covariance intensity of zero-mean
white noise independent of the initial state, with $t,t'\geq t_0$.
